...
from functions import flash, flash_rainbow

class CenterCascadeRainbow(Scene):

    def __init__(self, leds: List[List[LedCell]], startingFrame: int, frameDuration: int, reverse: bool, startCallback: Callable = None):
        self.leds = leds
    ...
    def run(self, frame: int):
        ...
        for row in self.leds:
            for cell in row:
                ...
                flash_rainbow.run(cell, self.cellDuration, frame - delay)
